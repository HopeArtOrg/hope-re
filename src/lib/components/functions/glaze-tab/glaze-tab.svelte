    ...
    <div class="flex flex-col gap-8 flex-1 min-h-0">
      {#if protection.hasResult && !protection.modelUsed}
        <div class="flex items-center gap-3 p-4 blue-note text-sm text-amber-700 animate-in fade-in duration-700 relative">
          ...
          <span>{t("glaze.basicApplied")}</span>
          <div class="absolute -top-2 -right-2 rotate-12 text-xs bg-white text-black px-2 py-0.5 shadow-sm border-2 border-foreground/10 doodle-line decorative-doodle">{t("glaze.phew")}</div>
        </div>
      {/if}
      ...
      {#if protection.resultImage}
        <div class="relative group">
          <BaseImagePlaceholder imageSrc={protection.resultImage}
                                label={t("placeholders.renderedCanvas")}
  ...
  {:else}
    {#if protection.hasResult && !protection.modelUsed}
      <div class="flex items-center gap-3 p-4 blue-note text-sm text-amber-700 animate-in fade-in duration-700 max-w-2xl mx-auto w-full relative">
        <TriangleAlertIcon class="size-4 shrink-0" />
        <span>{t("glaze.basicApplied")}</span>
        <div class="absolute -top-2 -right-2 rotate-12 text-xs bg-white text-black px-2 py-0.5 shadow-sm border-2 border-foreground/10 doodle-line uppercase font-bold decorative-doodle">{t("glaze.headsUp")}</div>
      </div>
    {/if}
    ...
    <div class="grid grid-cols-2 gap-10 flex-1">
      ...
      <div class="relative group">
        ...
        <div class="absolute -bottom-4 -right-4 rotate-15 text-base bg-primary text-primary-foreground px-4 py-1 shadow-md border-2 border-primary-foreground/20 doodle-line decorative-doodle">{t("glaze.wow")}</div>
